...
must_haves:
  ...
  key_links:
    ...
    - from: "src/container.py"
      to: "src/adapters/api/tmdb_client.py"
      via: "Provider Singleton TMDBClient avec api_key et cache"
      pattern: "TMDBClient|tmdb_client"
    - from: "src/container.py"
      to: "src/adapters/api/tvdb_client.py"
      via: "Provider Singleton TVDBClient avec api_key et cache"
      pattern: "TVDBClient|tvdb_client"
    - from: "src/container.py"
      to: "src/adapters/api/cache.py"
      via: "Provider Singleton APICache"
      pattern: "APICache|api_cache"
---
...
10. Methode `list_validated() -> list[PendingValidation]`:
    - Retourne les PendingValidation avec validation_status=VALIDATED
    - Utiliser pending_repo pour filtrer ou lister puis filtrer en Python
...
7. **Tests list_validated:**
   - test_list_validated_returns_validated_only: verifie filtrage par status
...
`pytest tests/unit/services/test_validation.py -v` passe avec tous les tests verts.
  ...
  <done>
Suite de tests complete avec couverture des scenarios auto-validation, validation manuelle, recherche, rejection et list_validated.
  </done>
</task>

<task type="auto">
  <name>Task 3: Integrer ValidationService et clients API au container DI</name>
  <files>src/container.py</files>
  <action>
Ajouter les providers pour les clients API et ValidationService.

**Approche choisie: providers.Singleton avec api_key directe.**
Le ValidationService gere deja les clients None dans ses methodes (retourne [] ou None).
...
2. **Ajouter le provider APICache (Singleton):**
   ```python
...
3. **Ajouter les providers pour les clients API:**
   ```python
   # Clients API - crees avec api_key depuis config
   # Si api_key est None/vide, le client sera cree mais ValidationService
   # gere ce cas en verifiant client.api_key avant utilisation
...
4. **Ajouter le provider MatcherService (s'il n'existe pas):**
   ```python
   matcher_service = providers.Singleton(MatcherService)
   ```

5. **Ajouter le provider ValidationService:**
   ```python
...
6. **Supprimer les commentaires placeholder existants:**
...
Important:
- Les cles API (tmdb_api_key, tvdb_api_key) sont Optional[str] dans Settings
- Le ValidationService gere le cas ou client.api_key est None/vide dans ses methodes search_manual et search_by_external_id
  </action>
  <verify>
Container charge sans erreur:
`python -c "from src.container import Container; c = Container(); print('OK')"`

Verification acces aux providers:
`python -c "from src.container import Container; c = Container(); print(c.api_cache); print(c.validation_service)"`
  </verify>
  <done>
ValidationService injectable via container.validation_service() avec clients API (tmdb_client, tvdb_client) et APICache cables.
  </done>
</task>
...
4. Verification des providers: `python -c "from src.container import Container; c = Container(); print(c.api_cache); print(c.tmdb_client); print(c.tvdb_client); print(c.validation_service)"`
5. Couverture: `pytest tests/unit/services/test_validation.py --cov=src/services/validation --cov-report=term-missing`
</verification>

<success_criteria>
- ValidationService existe avec toutes les methodes documentees incluant list_validated()
- APICache, TMDBClient, TVDBClient sont cables dans le container
- Tests couvrent les scenarios auto-validation (seuil 85%, unicite)
- Tests couvrent validation manuelle et recherche
- Service integre au container DI avec clients API
