...
must_haves:
  truths:
    ...
    - "La commande validate permet de valider un fichier specifique par ID"
    - "Les options --verbose/-v et --quiet/-q ajustent le niveau de detail"
    - "Le mode --dry-run simule sans modifier les fichiers"
  artifacts:
    - path: "src/main.py"
      provides: "CLI principale avec commandes process, pending, validate et callback verbose/quiet"
    ...
    - path: "tests/unit/adapters/cli/test_commands.py"
      provides: "Tests des nouvelles commandes"
      contains: "test_process"
  key_links:
    - from: "src/main.py"
      to: "src/adapters/cli/commands.py"
      via: "app.add_typer(validate_app) ou import direct"
      pattern: "validate_app|@app.command"
...
<objective>
Ajouter les commandes CLI principales (process, pending, validate) au point d'entree CineOrg
...
# Etat global pour les options de verbosire
...
<task type="auto">
  <name>Task 3: Implementer la commande validate et tests</name>
  <files>src/adapters/cli/commands.py, src/main.py, tests/unit/adapters/cli/test_commands.py</files>
  <action>
1. Ajouter commande validate (fichier specifique par ID):
```python
@app.command()
def validate(
    ...
    asyncio.run(_validate_single_async(file_id))

async def _validate_single_async(file_id: str) -> None:
    container = Container()
    await container.database.init()
    validation_svc = container.validation_service()
    ...
    # Note: Il faudra peut-etre ajouter cette methode au service/repo
...
Note importante: La validation par ID necessite d'ajouter une methode `get_pending_by_id` au ValidationService et au repository. Ajouter:

Dans `src/infrastructure/persistence/repositories/pending_validation_repository.py`:
```python
...
Dans `src/services/validation.py`:
```python
...
2. Dans main.py, importer et ajouter validate:
```python
from src.adapters.cli.commands import validate_app, process, pending, validate as validate_single

# Le nom 'validate' est deja pris par validate_app, donc utiliser un alias
app.command(name="validate-file")(validate_single)
# OU monter validate_app et garder validate comme sous-commande
```

Alternative plus propre: garder validate comme sous-commande de validate_app:
```python
@validate_app.command("file")
def validate_file(file_id: ...): ...
# Accessible via: cineorg validate file <ID>
```

3. Ecrire les tests dans test_commands.py:
```python
# Tests pour process
...
# Ou selon la structure choisie

python -m src.main --help
# Doit lister process, pending, validate
```
  </verify>
  <done>
- La commande validate permet de valider un fichier par ID
- Le repository a la methode get_by_id
- Le service expose get_pending_by_id
- Tests couvrent les nouvelles commandes (process, pending, validate)
...
- [ ] Commande validate permet de valider un fichier par ID
- [ ] Mode --dry-run simule sans modifier les fichiers
