...
must_haves:
  truths:
    ...
    - "Les erreurs de rate limiting sont recuperees automatiquement"
...
<task type="auto">
  <name>Task 2: Implementer APICache avec TTL differencies (TDD)</name>
  <files>
    - src/adapters/api/cache.py
    - tests/unit/adapters/api/test_cache.py
  </files>
  <action>
**TDD Sequence - RED phase first:**

1. Creer tests/unit/adapters/api/test_cache.py avec tests (AVANT implementation):
...
2. Executer `pytest tests/unit/adapters/api/test_cache.py -v` - les tests doivent ECHOUER (RED)

**GREEN phase:**

3. Creer src/adapters/api/cache.py avec la classe APICache:
   ```python
...
4. Executer `pytest tests/unit/adapters/api/test_cache.py -v` - les tests doivent PASSER (GREEN)
  </action>
  <verify>
...
<task type="auto">
  <name>Task 3: Implementer le decorateur retry avec backoff exponentiel (TDD)</name>
  <files>
    - src/adapters/api/retry.py
    - tests/unit/adapters/api/test_retry.py
  </files>
...
1. Creer tests/unit/adapters/api/test_retry.py avec tests (AVANT implementation):
...
2. Executer `pytest tests/unit/adapters/api/test_retry.py -v` - les tests doivent ECHOUER (RED)

**GREEN phase:**

3. Creer src/adapters/api/retry.py:
   ```python
...
4. Executer `pytest tests/unit/adapters/api/test_retry.py -v` - les tests doivent PASSER (GREEN)
  </action>
  <verify>
