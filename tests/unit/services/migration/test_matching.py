...
# ---- Routage film/série priorité au path utilisateur ---------------------


@pytest.mark.asyncio
async def test_path_routes_to_series_even_when_guessit_says_movie():
    """Un fichier guessit-MOVIE rangé dans Vidéothèque10/Séries/ → search_tv."""
    matcher = _make_matcher(
        parsed=_parsed(
            "Rivalité de génies", year=2015, media_type=MediaType.MOVIE
        ),
        tmdb_tv_results=[
            _result("123", "Rivalité de génies", year=2015, source="tmdb_tv"),
        ],
    )
    # media_root="Vidéothèque10" mais relative_category="Séries"
    cand = MigrationCandidate(
        symlink_path=Path("/old/Vidéothèque10/Séries/rivalite.mp4"),
        target_path=Path("/old/Vidéothèque10/Séries/rivalite.mp4"),
        media_root="Vidéothèque10",
        relative_category="Séries",
        size_bytes=1000,
        is_broken=False,
        already_on_destination=False,
        is_symlink=False,
...
@pytest.mark.asyncio
async def test_path_routes_to_movie_even_when_guessit_says_series():
    """Un fichier guessit-SERIES rangé dans Films/ → search films."""
    matcher = _make_matcher(
        parsed=_parsed(
            "Avatar", year=2009, media_type=MediaType.SERIES
        ),
        tmdb_results=[_result("19995", "Avatar", year=2009)],
    )
    cand = MigrationCandidate(
        symlink_path=Path("/old/Vidéothèque/Films/Avatar.mkv"),
        target_path=Path("/old/Vidéothèque/Films/Avatar.mkv"),
        media_root="Vidéothèque",
        relative_category="Films/SF",
        size_bytes=1000,
        is_broken=False,
        already_on_destination=False,
        is_symlink=False,
...
@pytest.mark.asyncio
async def test_path_routes_animations_to_series():
    """Le dossier Animations/ déclenche le routage série (TMDB+TVDB)."""
    matcher = _make_matcher(
        parsed=_parsed("Naruto", year=2002, media_type=MediaType.MOVIE),
        tmdb_tv_results=[_result("46260", "Naruto", year=2002, source="tmdb_tv")],
    )
    cand = MigrationCandidate(
        symlink_path=Path("/old/Animations/Naruto/ep1.mkv"),
        target_path=Path("/old/Animations/Naruto/ep1.mkv"),
        media_root="Animations",
        relative_category="Naruto",
        size_bytes=1000,
        is_broken=False,
        already_on_destination=False,
        is_symlink=False,
